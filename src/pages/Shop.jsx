...
import { HiOutlineShoppingCart, HiCheck, HiOutlineSearch } from 'react-icons/hi'; 
...
export default function Shop() {
  ...
  // Reads live global store data instead of a local timeout array
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-slate-900 py-10">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-end gap-6">
          ...
          {/* DYNAMIC SEARCH BAR WITH VECTOR ICON */}
          <div className="w-full md:w-80 relative flex items-center">
             <input 
                ...
                className="w-full bg-slate-800 border-none text-white rounded-xl py-3 pl-5 pr-11 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all"
             />
             <HiOutlineSearch className="absolute right-4 text-slate-400" size={20} />
          </div>
        </div>
      </div>
        ...
        <main className="flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProducts.length > 0 ? filteredProducts.map((p) => {
              ...
              // Calculates live status against global cart storage array
              const isAlreadyInCart = cart.some(item => item.id === p.id);
              ...
              return (
                <div key={p.id} className="group border border-slate-200 rounded-2xl p-4 hover:shadow-2xl transition-all flex flex-col h-full bg-white border-b-4 hover:border-b-blue-600">
                  ...
                  <div className="mt-4">
                    ...
                    {/* Dynamic state button that permanently stays green when inside the bag */}
                    <button 
                      onClick={() => !isAlreadyInCart && handleAddToCart(p)}
                      ...
                      className={`w-full py-3 rounded-xl font-black uppercase text-[10px] tracking-widest transition-all flex items-center justify-center gap-2 ${
                        isAlreadyInCart 
                          ? "bg-green-600 text-white cursor-not-allowed opacity-95 shadow-md shadow-green-100" 
                          : "bg-slate-900 text-white hover:bg-blue-600 shadow-md hover:shadow-blue-200 active:scale-95"
                      }`}
                    >
                      {isAlreadyInCart ? (
                        <>
                          <HiCheck size={14} className="text-white stroke-[3]" />
                          Added to Cart
                        </>
                      ) : (
                        <>
